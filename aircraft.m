function scopeHandles =  aircraftPositionEstimateExampleApp(genCode, ...
                                                    plotResults, numTSteps)
%AIRCRAFTPOSITIONESTIMATEEXAMPLEAPP Declare, initialize and step through
...
%
% This function aircraftPositionEstimateExampleApp is only in support of 
...
if plotResults
    load('AircraftPositionEstimateScopes.mat', 'XScope', 'YScope', ...
                                           'XErrorScope', 'YErrorScope');
    screen = get(0,'ScreenSize');
    outerSize = min((screen(4)-40)/2, 512);
    XScope.Position = [8, screen(4)-outerSize+8, outerSize-16, outerSize-92];
    YScope.Position = [outerSize+8, screen(4)-outerSize+8, outerSize-16, outerSize-92];
    XErrorScope.Position = [8, screen(4)-2*outerSize+8, outerSize-16, outerSize-92];
    YErrorScope.Position = [outerSize+8, screen(4)-2*outerSize+8, outerSize-16, outerSize-92];
else
    XScope = [];
    YScope = [];
    XErrorScope = [];
...
tuningUI = HelperCreateParamTuningUI(param, 'Thrust and Variance of Measurement Noise');
...
meanX = dsp.Mean('RunningMean', true);
meanY = dsp.Mean('RunningMean', true);
...
while (1)
    ...
    estErrorX = step(meanX, abs((filteredX-trueX)/trueX));
    estErrorY = step(meanY, abs((filteredY-trueY)/trueY));
    
    % Plot on time scopes
    if plotResults
        step(XScope, noisyX, trueX, filteredX);
        step(YScope, noisyY, trueY, filteredY);
        step(XErrorScope, 20*log10(estErrorX));
        step(YErrorScope, 20*log10(estErrorY));
    end
...
scopeHandles.XScope = XScope;
scopeHandles.YScope = YScope;
scopeHandles.XErrorScope = XErrorScope;
scopeHandles.YErrorScope = YErrorScope;
